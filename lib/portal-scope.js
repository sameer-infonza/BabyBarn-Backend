...
/**
 * Repair staff-role users incorrectly stored under CUSTOMER portalScope (legacy bug).
 * Frees the CUSTOMER email slot so a shop account can be created with the same address.
 * @returns {Promise<boolean>} true if a row was moved to STAFF
 */
export async function rehomeMisScopedStaffToStaffPortal(prisma, email) {
  const normalized = normalizeAuthEmail(email);
  if (!normalized) return false;

  const customerSlot = await prisma.user.findUnique({
    where: { email_portalScope: { email: normalized, portalScope: PORTAL_SCOPE.CUSTOMER } },
    include: { role: true },
  });
  if (!customerSlot || !isStaffRoleName(customerSlot.role?.name)) return false;

  const staffSlot = await prisma.user.findUnique({
    where: { email_portalScope: { email: normalized, portalScope: PORTAL_SCOPE.STAFF } },
    select: { id: true },
  });
  if (staffSlot) {
    // Cannot move — STAFF already occupied. Leave as-is; caller decides.
    return false;
  }

  await prisma.user.update({
    where: { id: customerSlot.id },
    data: { portalScope: PORTAL_SCOPE.STAFF },
  });
  return true;
}
