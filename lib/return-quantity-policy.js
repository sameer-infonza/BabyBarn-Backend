...
/**
 * Units still returnable on a line.
 * Cancelled lines (cancelledAt set) are never returnable — full-line cancel only today.
 * Formula: purchased − claimed non-rejected returns (cancelled qty is the whole line when cancelledAt is set).
 *
 * @param {{
 *   quantity?: number | null;
 *   cancelledAt?: Date | string | null;
 *   returnRequests?: Array<{ status: string; quantity?: number | null }>;
 * }} orderItem
 */
export function returnableQuantityForLine(orderItem) {
  if (orderItem?.cancelledAt) return 0;
